...
let botonCaracteristicas = document.getElementsByClassName("btn");
botonCaracteristicas.addEventListener("click", verCaracteriasticas, false);

function inicializarEventos(){
	let categorias = document.getElementById("categorias");
	let productos = document.getElementById("productos");
	let listado = document.getElementById("caracteristicas");
	productos.innerHTML="";
	listado.innerHTML = "";
	...
	for (let i = 0; i < CATEGORIAS.length; i++) {
		...
		let h1Categorias = document.createElement("h1");
		h1Categorias.innerHTML = obtenerIdCategorias(CATEGORIAS[i].id).nombre;
		divCategorias.appendChild(h1Categorias);
